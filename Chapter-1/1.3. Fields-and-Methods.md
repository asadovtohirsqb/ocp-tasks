...
The method name is `numberVisitors`. There's one parameter named month, which is of type `int`, which is a numeric type. Therefore, the <u>method signature</u> is `numberVisitors(int)`.
