...
> You might wonder why there are three options for the `classpath`. The `-cp` option is the short form. Developers frequently choose the short form because _we are lazy typists_. The `-classpath` and `--class-path` versions can be clearer to read but require more typing.
